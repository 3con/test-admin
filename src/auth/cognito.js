...
import { userPool, USERPOOL_ID, IDENTITY_POOL_ID } from '../config/aws';
import uuid from 'node-uuid';

// https://github.com/aws/amazon-cognito-js/
// entire cognito sync
//import AWS from 'amazon-cognito-js';
import { Config, AWS } from "aws-sdk";
	...
	const p = new Promise((res, rej) => {
		...
		authenticateUser(cognitoUser, authenticationDetails)
			...
			.catch((err) => {
				...
				console.log('rejecting due to failure');
				rej(err);
			});
	});
	return p;
	...
	const p = new Promise((res, rej) => {
		cognitoUser.authenticateUser(authenticationDetails, {
			onSuccess: function (result) {
				...
				const loginsObj = {
					// For the object's key name, use the USERPOOL_ID taken from our shared aws config js file
            ...
            //cognitoUser.completeNewPasswordChallenge('C4t4c0mb!@#$', attributes, this);
        }
	  });
	});
	return p;
